...
theorem imo_2024_p6
  ...
  · /- Let f be an aquaesulian function with f(0) = 0.
    ...
    · -- If f(x) + f(-x) = 0 for all x, we are done.
      ...
      suffices:{U|∃examples6 : ℚ, u examples6 + u (-examples6)= U} ⊆{0,(u (a : Rat)+ (u<|@@↑(( (-a ))))) } ..
      · constructor
        · exact ( Set.toFinite ( _) ).subset (by simpa using this)
        · exact (Set.ncard_le_ncard (by simpa using this)).trans (Set.ncard_pair (Ne.symm (↑ ( (c)) ) )).le
